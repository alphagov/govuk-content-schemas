...
module SchemaGenerator
  module SortedHash
    refine Hash do
      def deep_sort(&block)
        recrusive_deep_sort(self, &block)
      end

    private

      def recrusive_deep_sort(property, parent_key = nil, &block)
        if property.is_a?(Hash)
          children_sorted = property.each_with_object({}) do |(k, v), memo|
            memo[k] = recrusive_deep_sort(v, k, &block)
          end
          ...
          property.map { |item| recrusive_deep_sort(item, nil, &block) }
        else
          property
        end
      end
  ...
  class Schema
    ...
    def self.ordered_schema(schema_hash)
      ...
      sorted_hash = schema_hash.deep_sort do |(a), (b), parent_key|
        a, b = [a.to_s, b.to_s]
