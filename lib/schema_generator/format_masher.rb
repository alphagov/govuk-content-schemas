...
module SchemaGenerator
  class FormatMasher
    ...
    def notification_schema
      schema = base_schema.merge(
        required: notification_required_fields,
    ...
    def frontend_schema
      schema = base_schema.merge(
        required: frontend_required_fields,
  ...
  private
    ...
    def notification_required_fields
      content_schema_required_fields + ["expanded_links", "govuk_request_id"]
    end

    def frontend_required_fields
      content_schema_required_fields + ["expanded_links", "govuk_request_id"]
    end

    def notification_properties
      customisable = {
        base_path: schema_property(data["base_path"], "$ref": "#/definitions/absolute_path", true),
        title: schema_property(data["title"], type: "string", true),
        description: schema_property(data["description"], type: "string", true),
        routes: routes_property(data["routes"]),
        redirects: redirects_property(data["redirects"]),
        details: schema_property(data["details"], "$ref": "#/definitions/details", true),
        document_type: document_type_property(data["document_type"]),
        rendering_app: schema_property(data["rendering_app"], "$ref": "#/definitions/rendering_app_name", true),
      }.delete_if { |_, v| v.nil? }

      customisable.merge(consistent_notification_properties)
    end
    ...
    def consistent_notification_properties
      Jsonnet.load("jsonnet_formats/shared/consistent_notification_properties.jsonnet")
    end

    def schema_property(definition, default, forbidden_null = false)
      definition_hash = definition_as_hash(definition)
      return (forbidden_null ? { type: "null" } : nil) if definition_hash["forbidden"]
